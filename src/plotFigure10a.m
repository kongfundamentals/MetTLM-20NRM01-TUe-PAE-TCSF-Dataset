%% function plotFigure10a
%
% Description:
%   Generates and saves Figure 10a from the associated publication. This script
%   creates a comprehensive plot, comparing the results of the current study
%   with several datasets curated from the existing literature.
%
% Inputs:
%   - Reads all pre-analyzed data from: '../data/processedData/literatureAnalysisResults.mat'
%
% Outputs:
%   - Saves the figure to:          '../output/fig10a_twoColumn.emf'
%   - Also saves a PNG version to:  '../output/fig10a_twoColumn.png'
%
% Programmer:
...
function plotFigure10a
    ...
    % --- Kong et al. 2023 ---
    h(2) = plot(ax, litData.kong2023_red.xData, litData.kong2023_red.yData, symbols(2), 'MarkerSize', markerSize, 'MarkerEdgeColor', colors{2}, 'MarkerFaceColor', 'w');
    plot(ax, litData.kong2023_red.xFit, litData.kong2023_red.yFit, '--', 'Color', colors{2}, 'LineWidth', lineWidth);
    h(3) = plot(ax, litData.kong2023_green.xData, litData.kong2023_green.yData, symbols(3), 'MarkerSize', markerSize, 'MarkerEdgeColor', colors{3}, 'MarkerFaceColor', 'w');
    plot(ax, litData.kong2023_green.xFit, litData.kong2023_green.yFit, '--', 'Color', colors{3}, 'LineWidth', lineWidth);
    h(4) = plot(ax, litData.kong2023_warmWhite.xData, litData.kong2023_warmWhite.yData, symbols(4), 'MarkerSize', markerSize, 'MarkerEdgeColor', colors{1}, 'MarkerFaceColor', 'w');
    plot(ax, litData.kong2023_warmWhite.xFit, litData.kong2023_warmWhite.yFit, '--', 'Color', colors{1}, 'LineWidth', lineWidth);

    % --- Wang et al. 2017 ---
    h(5) = plot(ax, litData.wang2017_20deg.xData, litData.wang2017_20deg.yData, symbols(5), 'MarkerSize', markerSize, 'MarkerEdgeColor', colors{1}, 'MarkerFaceColor', 'w');
    h(6) = plot(ax, litData.wang2017_40deg.xData, litData.wang2017_40deg.yData, symbols(6), 'MarkerSize', markerSize, 'MarkerEdgeColor', colors{1}, 'MarkerFaceColor', 'w');

    % --- Yu et al. 2018 (Sin & Square) ---
    h(7) = plot(ax, litData.yu2018_sin.xData, litData.yu2018_sin.yData, symbols(7), 'MarkerSize', markerSize, 'MarkerEdgeColor', colors{1}, 'MarkerFaceColor', 'w');
    plot(ax, litData.yu2018_sin.xFit, litData.yu2018_sin.yFit, '--', 'Color', colors{1}, 'LineWidth', lineWidth);
    h(8) = plot(ax, litData.yu2018_square.xData, litData.yu2018_square.yData, symbols(8), 'MarkerSize', markerSize, 'MarkerEdgeColor', colors{1}, 'MarkerFaceColor', 'w');
    plot(ax, litData.yu2018_square.xFit, litData.yu2018_square.yFit, '--', 'Color', colors{1}, 'LineWidth', lineWidth);

    % --- Yu et al. 2018 (Colors) ---
    h(9)  = plot(ax, litData.yu2018_blue.xData, litData.yu2018_blue.yData, symbols(9), 'MarkerSize', markerSize, 'MarkerEdgeColor', colors{5}, 'MarkerFaceColor', 'w');
    h(10) = plot(ax, litData.yu2018_red.xData, litData.yu2018_red.yData, symbols(10), 'MarkerSize', markerSize, 'MarkerEdgeColor', colors{2}, 'MarkerFaceColor', 'w');
    h(11) = plot(ax, litData.yu2018_white.xData, litData.yu2018_white.yData, symbols(11), 'MarkerSize', markerSize, 'MarkerEdgeColor', colors{1}, 'MarkerFaceColor', 'w');

    % --- CIE 249:2022 & Tan et al. 2024 ---
    h(12) = plot(ax, litData.cie2022_from_tan2024.xFit, litData.cie2022_from_tan2024.yFit, [symbols(2) '-'], 'MarkerSize', markerSize/2, 'MarkerEdgeColor', colors{1}, 'MarkerFaceColor', 'w', 'Color', colors{1});
    h(13) = plot(ax, litData.tan2024.xFit, litData.tan2024.yFit, [symbols(2) '-'], 'MarkerSize', markerSize/2, 'MarkerEdgeColor', colors{1}, 'MarkerFaceColor', colors{1}, 'Color', colors{1});
    ...
    legend(ax, h, ...
        {'Current study', ...
        '^{22} red', '^{22} green', '^{22} warm white',...
        '^{14} 20^{\circ}', '^{14} 40^{\circ}', ...
        '^{17} sin', '^{17} square', ...
        '^{17} blue', '^{17} red', '^{17} white', ...
        '^{ 1} CIE 249:2022', '^{27} PAVM'}, ...
